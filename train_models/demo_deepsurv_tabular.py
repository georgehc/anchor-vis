...
hyperparams = \
    [(hypersphere, n_layers, n_nodes, batch_size, max_n_epochs, lr)
     for n_layers
    ...
    for dataset in datasets:
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam_idx, hyperparam in enumerate(hyperparams):
                ...
                layers = make_relu_mlp(
                    n_layers, X_proper_train_std.shape[1], n_nodes, n_nodes)
                if hypersphere > 0:
                ...
                for epoch_idx in range(max_n_epochs):
                    ...
                    new_hyperparam = \
                        (hypersphere, n_layers, n_nodes, batch_size,
        ...
        hypersphere, n_layers, n_nodes, batch_size, n_epochs, lr, seed \
            = arg_min
        ...
        np.savetxt(os.path.join(cached_embeddings_dir, 'test_X.txt'), X_test)
